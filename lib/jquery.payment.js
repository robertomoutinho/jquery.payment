...
(function() {
  ...
  reFormatCardNumber = function(e) {
    return setTimeout(function() {
      ...
      $target.val(value);
      setCaretPosition($target.attr('id'), $target.prop('selectionEnd'));
      return;
    });
  };
    ...
    if (re.test(value)) {
      ...
      return setTimeout(function() {
        ...
        if(e.which === 8){
            ...
            setCaretPosition($target.attr('id'), $target.val().length);
        }
        return;
      },0);
  ...
  formatBackCardNumber = function(e) {
    ...
    $target = $(e.currentTarget);
    value = $target.val();
    // only execute the bellow code when backspace is pressed
    if (e.which !== 8) {
      return;
    }
    if (($target.prop('selectionStart') != null) && $target.prop('selectionStart') !== value.length) {
...
//  this.on('input', reFormatCardNumber); // no reformating the field when backspace is used
    this.on('input', setCardType);
    return this;
  };
  
  $.payment.fn.formatCpf = function() {
    this.on('keypress', restrictNumeric);
    this.on('keypress', formatCpf);
    this.on('keydown', formatCpf);
    this.on('paste', formatCpf);
    this.on('change', formatCpf);
    this.on('input', formatCpf);
    return this;
  };
  ...
  $.payment.fn.restrictLetters = function() {
    this.on('keypress', restrictLetters);
    this.on('paste', reFormatLetters);
    this.on('change', reFormatLetters);
    this.on('input', reFormatLetters);
    return this;
  };
